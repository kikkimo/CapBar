...
首次启动会加入默认的 `~/.claude` 和 `~/.codex` 目录。其他账号在“账号与设置”中添加配置目录。默认只读取上次快照；点“全部刷新”会刷新所有空闲账号，账号行的按钮只刷新该账号。右键菜单栏图标可以全部刷新、进入配置或退出 CapBar；设置页的“关于 CapBar”提供项目仓库链接。可选开启“打开弹窗时自动刷新”，默认阈值为 5 分钟，按账号分别判断。弹窗关闭后，正在进行的刷新仍会完成并保存结果。
