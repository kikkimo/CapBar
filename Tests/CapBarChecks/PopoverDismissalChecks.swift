...
@MainActor func runPopoverDismissalChecks() {
    ...
    check(dismissal.shouldDismissOutsideClick(isShown: true), "outside click dismisses a visible popover")
    check(!dismissal.shouldDismissOutsideClick(isShown: false), "outside click does nothing when popover is hidden")
    ...
    check(!dismissal.shouldDismissOutsideClick(isShown: true), "outside click does not dismiss while the folder picker is open")
